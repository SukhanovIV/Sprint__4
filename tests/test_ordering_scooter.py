...
class TestOrderingScooter:
    ...
    def test_order_via_the_top_button(self, driver, example_1):
        ...
        with allure.step('Дожидаемся видимости кнопки "Заказать"'):
            WebDriverWait(driver, 5).until(ec.visibility_of_element_located(RentLocators.order))
        with allure.step('Выбор данных об аренде 1'):
            RentPage(driver).rental_option_1()
        with allure.step('Кликаем "Заказать'):
            RentPage(driver).clicking_order()
        with allure.step('Дожидаемся видимости кнопки "Да", для подтверждения заказа'):
            WebDriverWait(driver, 5).until(ec.visibility_of_element_located(RentLocators.button_yes))
        with allure.step('Кликаем на кнопку "ДА", подтверждая заказ'):
            RentPage(driver).clicking_button_yes()
        assert WebDriverWait(driver, 5).until(ec.visibility_of_element_located(RentLocators.view_status))
    ...
    def test_order_via_the_down_button(self, driver, example_2):
        ...
        with allure.step('Дожидаемся видимости нижней кнопки "Заказать"'):
            WebDriverWait(driver, 5).until(ec.visibility_of_element_located(MainLocators.order_down))
